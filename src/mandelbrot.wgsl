...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var x = min_x + (params.x_range * f32(pixel.x)) / f32(params.width);
    var y = max_y - (params.y_range * f32(pixel.y)) / f32(params.height);

    var old_x = x;
    var old_y = y;

    let pixel_index = pixel.x * params.width + pixel.y;
