...
const WORKGROUP_SIZE: u64 = 16 * 16;
const WIDTH: usize = (WORKGROUP_SIZE * 16 * 4) as usize;
const HEIGHT: usize = (WORKGROUP_SIZE * 16 * 4) as usize;

const WGPU_MAX_BUFFER_SIZE: u64 = 4 * 1024 * 1024 * 1024;
const WGPU_MAX_STORAGE_BUFFER_BINDING_SIZE: u32 = 2 * 1024 * 1024 * 1024;
...
async fn gpu_device_queue() -> (wgpu::Device, wgpu::Queue) {
    ...
    let limits = wgpu::Limits {
        max_buffer_size: WGPU_MAX_BUFFER_SIZE,
        max_storage_buffer_binding_size: WGPU_MAX_STORAGE_BUFFER_BINDING_SIZE,
        ..Default::default()
    };
...
async fn run() {
    ...
    let cpu_buf = cpu_buffer(&device, (WIDTH * HEIGHT * std::mem::size_of::<u32>()) as u64).await;
    let gpu_buf = gpu_buffer(&device, (WIDTH * HEIGHT * std::mem::size_of::<u32>()) as u64).await;
...

    {
        ...
        cpass.dispatch_workgroups((WIDTH as u64 / 16) as u32, (HEIGHT as u64 / 16) as u32, 1);
    }
